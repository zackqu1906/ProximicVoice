...
from .edit_operations import apply_edit_response
from .edit_tool import EDIT_PLAN_TOOL
...
class OpenAICompatibleTextProcessor:
    ...
    def process_with_attempts(
        ...
        if normalized_mode == INPUT_MODE_EDIT:
            ...
            # max_tokens = max(256, min(2048, len(target) * 2 + 256))
            max_tokens = 2048
        else:
            system_prompt = DICTATION_PROMPT
            user_content = raw_text
            # max_tokens = max(64, min(512, len(raw_text) * 2 + 64))
            max_tokens = 2048
        if normalized_mode == INPUT_MODE_EDIT:
    ...
    def _process_edit_with_retry(
        ...
        *,
        ...
        for attempt in range(2):
            ...
            try:
                ...
                response = self._request_chat(
                    ...
                    system_prompt=(
                        system_prompt
                        + self._edit_retry_prompt(retry_error, retry_output)
                        if retrying
                        else system_prompt
                    ),
                    user_content=user_content,
            ...
            try:
                final_text = apply_edit_response(target, response)
            except Exception as exc:
                if not retrying:
                    retry_error = str(exc)
        ...
        raise AssertionError("编辑计划重试流程异常结束")

    @staticmethod
    def _edit_retry_prompt(validation_error: str, invalid_output: str) -> str:
        safe_error = str(validation_error or "工具参数缺失或格式不正确")[:1000]
        safe_output = str(invalid_output or "（没有可用的工具参数）")[:4000]
        return EDIT_PLAN_RETRY_PROMPT.format(
    ...
    def _request_chat(
        ...
        *,
        ...
        if use_ark_responses:
            ...
            if require_edit_plan:
                function = EDIT_PLAN_TOOL["function"]
                request_body["tools"] = [
                    {
                        "type": "function",
                        "name": function["name"],
            ...
            if require_edit_plan:
                request_body["tools"] = [EDIT_PLAN_TOOL]
                if provider == LLM_PROVIDER_LOCAL:
                    # llama.cpp uses the string form to build a constrained
                    # tool-call grammar.  There is exactly one edit tool, so
                    # "required" also uniquely selects submit_text_edit_plan.
        ...
        if require_edit_plan and provider == LLM_PROVIDER_LOCAL:
            content = self._extract_required_local_tool_arguments(payload)
            if content is None:
                raw_payload = json.dumps(payload, ensure_ascii=False, indent=2)
                raise LLMResponseProcessingError(
                    "本地模型没有调用 submit_text_edit_plan 工具",
                    raw_payload[:4000],
                )
        else:
    ...
    @staticmethod
    def _extract_required_local_tool_arguments(
        payload: object,
        ...
        if not isinstance(tool_calls, list):
            return None
        expected_name = EDIT_PLAN_TOOL["function"]["name"]
