...
点击后会直接使用列表当前保存的 BLE 扫描对象连接，不再额外等待一次固定时长的二次扫描。
连接流程依次显示“正在连接设备”“正在验证设备音频”“正在加载检测模型”
“正在加载语音模型”和“准备就绪”。应用先确认设备能持续提供真实 PCM，再加载耗时的
检测与语音模型。设备连接后，“开启/暂停语音识别”只
控制检测与 ASR，不会断开 Ring；需要释放设备时单独点击“断开设备”。设备保持连接时
设置会暂时锁定，断开后可以修改。

首次连接会开启麦克风并验证真实 PCM。加载模型期间 MIC 保持开启，但收到的音频会被丢弃，
不会进入检测或 ASR；模型完成后要求收到新的 PCM 才进入准备状态。ADPCM 会由 SDK 解码
成 16 kHz PCM，但有损压缩仍可能改变 Stage2 分数分布。Opus 选项需要额外的 libopus
运行库。

音频流中断或 watchdog 发现设备不再传输时，应用会立即关闭连接，不会在后台重启麦克风
或自动重连。界面会保留上次选择的设备并显示“重新连接设备”；只有用户点击该按钮才会
重新尝试，也可以点击“选择其他设备”更换目标。Windows 和 macOS 使用相同的产品逻辑。
...
连接、NUS 服务或真实 PCM 验证失败时应用会自动关闭设备，无需用户再次点击断开。
...
confirm、cancel、retry 动作，无需修改 ASR、大模型或桌面适配层。
