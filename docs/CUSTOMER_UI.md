...
具体错误且不再提供“确认应用”，只保留“取消”。取消时已失败 Attempt 的原因自动标记为
`llm_error`，无需用户再按失败原因快捷键。任一分支先产生有效候选时会立即显示；另一分支只在后台完成数据采集，
不会阻塞预览和后续用户操作。

取消后，转写浮窗右侧会出现 10 秒的小型失败原因提示。当前临时快捷键为
`Alt+A`（语音识别错误）、`Alt+L`（大模型理解错误）和 `Alt+O`（其他原因）；这些键只在
原因提示有效期间被全局拦截。超时或先执行下一次确认、取消时，上一 Attempt 保持未标注。
连续取消产生新提示时，旧提示会先关闭 180 ms 再显示新提示，并从重新显示时开始
