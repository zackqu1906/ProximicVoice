...
当前没有可执行的操作、目标窗口不符合原有要求时，动作手势不执行。弹指默认不映射。

原生撤销入口独立于操作记录数量和浮窗超时：第一次快捷键没有生效时，点回原文本框
即可重试，不需要先说一句话来增加记录。每次触发仍检查目标焦点；断开设备会清除
保留目标。重复触发按顺序发送，不回读或复制文本。记录耗尽后的重试只写诊断日志。

主程序仍由近场 `ACTIVATE` 开始；由配置的确认手势（默认 tap）提交本句。连续 REJECT、
静默超时和 15 秒上限在此模式下不再结束语音，未确认就持续收听。收听期间跳过后续
Stage2 推理，继续向 ASR 发送音频。确认手势从 SDK 手势线程直接请求音频线程结束，
在下一次音频处理时提交，不经过 GUI 动作队列，也不等待新的检测结果。重复确认、空闲或
处理期间的确认手势无效。暂停、取消、断开会丢弃未确认语音，不自动提交。
结束时先关闭下一句的接收，再提交 final；处理成功或报错后沿用原流程恢复。

主程序共用原有蓝牙连接接收 200 Hz IMU，在独立线程分类；语音处理暂时挂起时，
手势仍可用于取消、撤销或转换。启用手势后，语音数据集的 IMU 记录及元数据均为
200 Hz；未启用手势或 tap 结束模式的调用方仍沿用原采样配置。手势加载或推理出错时
会提示确认手势无法结束，可按 Esc 取消本句并重连，不会悄悄切回自动结束。
断开重连会建立新识别器，旧连接排队的手势不执行。

主程序日志保存在项目的 `logs/diagnostic.log`（也可在界面打开诊断日志目录）：

实时日志只显示简短的操作行，例如 `[手势] 左滑 → 取消`，以及状态变化时的异常
提示。周期统计、置信度、模型参数和完整事件字段只保留在诊断文件中；未分配的
手势及旧连接事件不刷实时日志。有效确认只显示一行，例如 `[手势] tap → 结束本句`。
...
- `[ASR] END reason=gesture-tap`：`tap_to_end_ms` 记录稳定手势回调到结束提交前的
  调度时间；不包含手势模型识别动作本身、ASR final 或 LLM 的时间，不能当成整条
  流程的延迟。字段名为兼容旧日志保留，也用于自定义确认手势。保存录音在 final ASR 入队之后进行。
