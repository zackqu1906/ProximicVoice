...
def test_comparison_sends_identical_instruction_input_to_every_model(capsys):
    ...
    class FakeProcessor:
        def process_with_trace(self, text, mode, settings, target_text):
            calls.append((text, mode, target_text, settings.model))
            suffix = "甲" if settings.model == "model-a" else "乙"
            return (
                f"修改后的完整文本-{suffix}",
                '{"kind":"rewrite","text":"修改后的完整文本-' + suffix + '"}',
            )
    ...
    assert "执行后的完整文本" in output
    assert "最终结果集中比较（2 个模型）" in output
...
def test_comparison_groups_all_models_with_elapsed_time(capsys):
    ...
    output = capsys.readouterr().out
    assert "最终结果集中比较（3 个模型）" in output
...
def test_retry_outputs_are_all_displayed(capsys):
    run = test_llm.ModelRunResult(
        ...
        success=False,
        elapsed_s=8.91,
        model_output='{"kind":"rewrite","text":两天"}',
        model_outputs=(
            '{"kind":"rewrite","text":一天"}',
            '{"kind":"rewrite","text":两天"}',
        ),
        error="大模型没有返回有效 JSON 编辑计划",
    )
    ...
    assert '{"kind":"rewrite","text":一天"}' in captured.out
    assert "第 2 次 function/tool arguments：失败" in captured.out
    assert '{"kind":"rewrite","text":两天"}' in captured.out
