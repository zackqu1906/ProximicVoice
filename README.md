...
- macOS 行内听写默认撤销当前语音事务，编辑取消还原为听写；设置中开启“多次撤销”后，可继续撤销同一输入框内更早的听写与编辑。手动输入、移动光标、切换目标或输入法会清除历史，关闭开关也会清除历史。详细规则和性能记录见 [多次撤销](docs/UNDO_HISTORY.md)。
- 写入和撤销均核验结果。未定稿撤销直接清除本句组合；微信与 Codex 定稿听写撤销只选中本句再退格，不全选全文或依赖 `Cmd+Z` 分组；其他应用通过输入法范围替换。
- Windows 保持原生 `Ctrl+Z` 路径，撤销粒度由目标应用决定；每次发送前检查焦点，断开设备后清除目标。原生撤销事件日志不代表应用已确认文字恢复。
- 明确的“删除全文”通过协议校验后直接执行，并进入同一个可撤销操作栈。
- 正常语句处理完成前暂停接收下一句；出现终止处理的错误提示后立即恢复识别。修改失败时仍可在提示消失或下一句话开始前选择“改用听写”；下一句话开始后，上一句的失败状态和待完成回调会失效。
...
- Windows 和 macOS 均支持跨应用听写、读取当前文本、直接修改、连续撤销和类型转换。
- macOS 支持处理期间全局 `Esc` 取消，但不支持 Windows 的右 `Alt` 按住说话和
  `Alt+1/Alt+2` 模式切换。
...
1. 点击顶栏“设置”，在独立窗口中选择 ASR 后端、语言和 CPU/GPU；可按需要调整只作用于 ASR/语音记录的输入增益。
...
8. 处理期间可按 `Esc` 取消；结果应用后可在目标文本框旁撤销或改用另一种语音类型。
...
- 任一路先通过校验就立即应用候选；另一条分支只在后台补全采集记录，不阻塞 UI。
- 片段协议只有选中完整原文并返回空片段时才会清空；全文协议返回空 `modified_text` 时也可表示清空。
...
LLM 候选通过结构与目标文本校验后会直接应用；每次写回都会进入操作栈，用户可在文本框旁连续撤销。

### 为什么“删除全文”得到 0 个字符？

合法删除全文的候选就是空字符串。程序校验明确的清空意图后执行，并保留撤销入口。
